...
def chatbot_response(user_input, threshold=0.6, fuzzy_threshold=0.6):
    user_input_lower = user_input.lower().strip()
    ctx = context.get_context()

    # ========================
    # Follow-up for weather
    # ========================
    if ctx.get("last_api_type") == "weather":
        ...
        if not city:
            ...
            if tokens:
                possible_city = tokens[-1].strip("?.!,")
                skip_words = {"about","what","how","the","is","and","in"}
                if possible_city not in skip_words and len(possible_city) > 2:
                    city = possible_city
        if city:
            context.set_context(entity=city)
    ...
    if confidence >= threshold:
        ...
        matches = difflib.get_close_matches(user_input_lower, all_patterns, n=1, cutoff=fuzzy_threshold)
        if matches:
            predicted_tag = pattern_to_tag[matches[0]]
        else:
    ...
    # ========================
    context.set_context(intent=predicted_tag)
    ...
    if predicted_tag == "weather":
        ...
        for key in ("GPE","LOC","FAC"):
            if key in entities and entities[key]:
                city = entities[key][0]
                break
        if not city and ctx.get("entity"):
            city = ctx.get("entity")
        if not city:
            tokens = user_input_lower.split()
            if "in" in tokens:
    ...
    # Default fallback
    return get_response_from_tag(predicted_tag)
